...
function NewRequestForm(props){
    //btw the sign up css is refgerenced from genbody
                ...
                <div id="submitButtonCont">
                    <button id="formSubmit" onClick={()=>{
                        ...
                        if(sessionCookie != null && name !== "" && phoneNum !== "" && nFeatures !== ""){
                            props.setPromptRequest({type:"set",msg:"Request Submitted. Please Wait for Server..."});
                            var curCenter = null;
                                ...
                                navigator.geolocation.getCurrentPosition((position)=>{
                                    ...
                                    apiRequest("http://localhost:8070/","",
                                        ...
                                        "POST").then((data)=>{
                                            console.log(data)
                                            setNFeatures(""); 
                                            setAdditionalInfo(""); setMeetUp("");
                                            if(data["code"] == 1){
                                                props.setPromptRequest({type:"set",msg:"Request Recieved"});
                                            }
                                        })
                                })
                            }
