        ...
        if(sessionCookie != null){
            ...
            .then((data)=>{
                if(data["code"] == 1){
                    ...
                    setTabDisplay(<LoginTab setTabState={setTabState}></LoginTab>)
                }
            })
        } else
            setTabDisplay(<LoginTab setTabState={setTabState}/>);
    },[])
    useEffect(()=>{
        if(tabState == 0){
            setTabDisplay(<LoginTab setTabState={setTabState}/>);
            ...
            setTabDisplay(<HistoryTab setTabState={setTabState}/>);
        }
        else if(tabState == 3){
            setTabDisplay(<FinishRequest setTabState={setTabState}/>);
        }
    },[tabState])
        ...
        <div id="infoTab">
            {(tabState == 0 ? <div></div> :
                <div id="infoTabTabCont">
                    <div className="tabOpt" onClick={()=>{setTabState(0)}}>
                        <img src={tmpOptImg} alt="login" className='optImg'></img>
                    </div>
